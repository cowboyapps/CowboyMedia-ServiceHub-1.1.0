...
**Never pass an explicit `{ scope: "/" }` to `serviceWorker.register("/sw.js")` on iOS.** A root-level worker (`/sw.js`) already defaults its scope to `/`, so the option is redundant — but on iOS/WebKit standalone PWAs (seen on iOS 18.x) the *explicit-scope* code path rejects a brand-new registration with `SecurityError: Scope URL should start with the given script URL`, even when `/sw.js` is served cleanly (HTTP 200, no redirect) WITH a `Service-Worker-Allowed: /` header. That path compares the scope against the *full script URL* (`"/"` does not start with `"/sw.js"`) and ignores the header; the default-scope path does neither. **Symptom vs cause:** the same `SecurityError` wording is *also* the textbook symptom of the worker script being HTTP-redirected before load (VPN / iCloud Private Relay / content-filter / config profile). Distinguish them with a `fetch("/sw.js", {redirect:"manual"})` probe — `opaqueredirect` ⇒ redirect/interceptor; `200/basic` ⇒ no redirect, so the cause is the explicit scope option, not the network. **Why it hid:** a long-standing install keeps its old worker alive, so a device that registered before the explicit-scope change keeps working and masks the regression; only a *fresh* register (clean reinstall, or a never-before-registered device) hits it. **Fix:** call `register("/sw.js")` with no options.
