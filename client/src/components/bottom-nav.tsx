...
import { Activity, MessageSquare, AlertTriangle, Newspaper, Menu, RefreshCw, Mail, FileText, Settings, Shield, LogOut, Download, Users, BookOpen } from "lucide-react";
...
export function BottomNav() {
  ...
  const overflowRoutes = ["/service-updates", "/messages", "/community", "/report-request", "/downloads", "/knowledge", "/settings", "/admin"];
      ...
      <Sheet open={moreOpen} onOpenChange={setMoreOpen}>
        <SheetContent side="bottom" className="rounded-t-2xl px-4 pt-3 pb-4" overlayStyle={{ bottom: "calc(3.5rem + env(safe-area-inset-bottom, 0px))" }} style={{ bottom: "calc(3.5rem + env(safe-area-inset-bottom, 0px))" }}>
          ...
          <div className="border-t mt-4 pt-4">
            ...
            <p className="text-[10px] text-muted-foreground/60 text-center mt-3" data-testid="text-sheet-version">Version 3.2</p>
          </div>
        </SheetContent>
      </Sheet>
    </>
