...
- **Customer Engagement**: News stories with photo support, and a customer message center with unread badges.
- **Conversational Messaging**: Two-way threaded messaging between admins and customers. `message_threads` table stores thread metadata (admin, customer, subject); `thread_messages` stores individual messages with read tracking. Admins create threads from Messages tab in admin portal; customers view/reply from `/messages` page. Real-time via WebSocket (scoped broadcasts to participants only), with typing indicators, date separators, smart auto-scroll. Push notifications always sent; email notifications suppressed when recipient is actively viewing the thread. Email templates: `customer_thread_message`, `admin_thread_message`. Routes: GET/POST `/api/message-threads`, GET `/api/message-threads/:id`, GET/POST `/api/message-threads/:id/messages`, PATCH `/api/message-threads/:id/read`, GET `/api/message-threads/unread-count`, DELETE `/api/message-threads/:id`. Master admin sees all threads. Migration: `migrations/004_message_threads.sql`.
