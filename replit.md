...
- **`/home/servicehub` permissions drift to root.** Any time someone runs `npm`, `pm2`, or anything else that writes under the app user's home as bare `root` (e.g. an emergency manual recovery), files under `/home/servicehub/.npm`, `~/.bash_profile`, `~/.npmrc`, etc. end up root-owned and the next `servicehub`-user `npm ci` dies with `EACCES`. `update.sh` now self-heals the **entire** `/home/servicehub` tree (not just `~/.npm`) at the very start of every deploy — before `pg_dump`, before `git fetch`, before `npm ci` — and aborts loudly if a recursive `chown` doesn't stick (immutable attr, broken mount, SELinux). Manual fix when it ever needs one: `sudo chown -R servicehub:servicehub /home/servicehub`. Avoid by **always** running `npm`/`pm2` via `sudo -u servicehub` or `sudo -u servicehub -H bash -lc '...'`, never as bare root. The `:x:` Discord failure embed now also appends a one-line remediation hint when this pattern is detected in the log tail (`EACCES` on `.npm` or `.bash_profile: Permission denied`), so on-call doesn't have to grep the runbook at 3am.
  - **Root-cause hunt status (Task #214, May 2026)**: in-repo audit is clean — every `npm`/`pm2` invocation in `install.sh`, `migrate.sh`, `rollback.sh`, `restore.sh`, `update.sh` correctly uses `sudo -u servicehub -H bash -lc`, and the listener's systemd unit has `ProtectHome=true` so it can't write to `/home/*` at all. The only in-repo paths that run something as root with HOME pointing at the app user's home are (a) `npm install -g pm2` in install.sh:88 and migrate.sh:219 (writes to `/usr/lib/node_modules`, not `/home/servicehub` — safe) and (b) `pm2 startup systemd -u servicehub --hp /home/servicehub` in install.sh:175 (one-time install only — could plausibly create `/home/servicehub/.pm2/...` as root on first install but doesn't recur). So the recurring source has to be **outside the repo**: an external cron, a sudoers rule we don't control, or past manual `sudo npm`/`sudo pm2` recoveries. To find it, run `sudo bash /opt/servicehub/deploy/diagnose-home-perms.sh` on the VPS — it's read-only, dumps current offenders + mtimes, audits sudoers, lists crons + systemd timers, greps `/root/.bash_history` for past root-run npm/pm2, and counts how often the self-heal has been firing. Paste output back to plan the actual fix.
