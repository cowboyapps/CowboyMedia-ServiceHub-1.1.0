...
- **Scrollable Header**: Header scrolls with content (not sticky), uses `bg-muted` background for visual separation in both light/dark themes. Centered CowboyMedia logo (`h-10` on mobile, `h-8` on desktop) links to dashboard (`/`). Mobile header shows a "Dashboard" pill button (left side) for quick home access. Desktop header shows SidebarTrigger on left + centered logo. OfflineBanner renders above PullToRefresh so it stays visible.
