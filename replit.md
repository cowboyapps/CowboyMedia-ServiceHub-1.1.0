...
- **Sticky Header**: Header stays fixed at top (outside scroll container) with `flex-shrink-0` and `min-h-[3rem]`, uses `bg-muted` background for visual separation in both light/dark themes. Centered CowboyMedia logo (`h-12` on mobile, `h-8` on desktop) links to dashboard (`/`). Mobile header shows a "Dashboard" rounded-rectangle button (left side, `rounded-lg`, Home icon) for quick home access. Desktop header shows SidebarTrigger on left + centered logo. OfflineBanner renders above PullToRefresh so it stays visible.
