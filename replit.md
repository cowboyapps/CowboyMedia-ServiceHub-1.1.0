...
- **Sticky Header**: Header stays fixed at top (outside scroll container) with `flex-shrink-0` and `min-h-[3rem]`, uses `bg-muted` background for visual separation in both light/dark themes. Outer container uses `h-dvh` (not `h-screen`) to prevent mobile browser address bar from causing outer page scroll. Centered CowboyMedia logo (`h-12` on mobile, `h-8` on desktop) links to dashboard (`/`). Mobile header shows a "Dashboard" rounded-rectangle button (left side, `rounded-lg`, Home icon) for quick home access. Desktop header shows SidebarTrigger on left + centered logo. OfflineBanner renders above PullToRefresh so it stays visible. iOS PWA notch safe area handled via a `bg-muted` spacer div with `height: env(safe-area-inset-top)` placed above OfflineBanner in both authenticated and unauthenticated layouts — fills notch area with header color while pushing content below. Body padding uses only horizontal safe-area insets (left/right); top inset handled per-layout, bottom handled by bottom-nav.
