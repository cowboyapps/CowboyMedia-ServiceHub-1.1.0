...
- **Environment Variables**:
    ...
    - `WHMCS_API_IDENTIFIER`, `WHMCS_API_SECRET` (optional — WHMCS billing API credentials. The base URL is NOT an env var; it lives in the `whmcs_settings` DB row and is edited in Admin Portal → WHMCS Billing. The integration no-ops when any of {identifier, secret, baseUrl} is missing.)
    ...
    - `CHANGELOG_APPEND_TOKEN` (long random string, **must be set on prod AND mirrored to Replit Secrets**). Bearer token the agent uses to append changelog bullets straight at production via `POST /api/agent/changelog/:version/append`. Without this, the agent can't satisfy the auto-append rule below — its appends would land in the dev DB and never reach customers. On the VPS it lives in `/opt/servicehub/.env` (the app's env file — NOT `/etc/servicehub.env`, which does not exist; `/etc/servicehub-deploy.env` is the separate deploy-listener file); the prod server reads it via PM2's process env. Rotation: `openssl rand -hex 32`, update Replit Secrets, update `/opt/servicehub/.env`, then `sudo -u servicehub pm2 reload servicehub --update-env` (remember to source the env file first — see "Manual deploy" below). The route fails closed (HTTP 503) when the env var is unset, so a typo'd or missing prod value blocks appends loudly instead of silently dropping them.
...
- **PM2 splits long log lines and trips the post-deploy log-tail gate.** The express request logger in `server/index.ts` embeds the JSON response body after `:: ` in each log line. PM2's log file storage has a ~1KB per-line buffer; longer lines get split into multiple physical lines, and only the first chunk carries the `[express]` prefix. Continuation chunks then slip past the gate's `grep -v "\[express\]"` filter — and any route that returns historical strings matching `column "x" does not exist` / `Migration error` / `relation .* does not exist` / `ECONNREFUSED` (e.g. `GET /api/admin/health/errors`, audit/activity logs) rolls back a perfectly healthy deploy. Two-pronged defense: (1) `server/index.ts` caps the embedded body to 200 chars so new occurrences never split, (2) `deploy/update.sh`'s log-tail gate also strips lines beginning with a JSON glyph (`"`, `,`, `:`, `{`, `}`) to handle residue already on disk. (Do NOT add `[` or `]` to that strip set — `[migrate]`/`[seed]` source prefixes are real crash signal.) Don't relax either half.
...
- **WHMCS Client**: `server/whmcs.ts` (stateless API wrapper + pure parsing helpers) + `server/whmcs-settings.ts` (admin settings handler)
...
- **WHMCS Billing Integration**: `server/whmcs.ts` is a stateless WHMCS API client mirroring the Telegram pattern — API identifier/secret from env secrets, base URL from the `whmcs_settings` DB singleton, never throws into handlers (returns tagged `{ok,...}` results), no-ops when unconfigured. Account linking stores `users.whmcs_client_id` (nullable, **UNIQUE**) + `users.whmcs_linked_at` — this is the cross-cutting contract downstream billing features build on (one WHMCS client ↔ at most one ServiceHub user). The admin customer-detail panel GET `/api/admin/users/:id/whmcs` is **PURE** (never writes, never 500s) and returns a **locked shape** `{ configured, enabled, link, linkedClient, suggestion }`; `linkedClient`/`suggestion` are null when WHMCS is unreachable. Auto-link by email happens via the separate POST `/api/admin/users/:id/whmcs/auto-match` (idempotent, 409 on conflict), which the frontend fires once when a `suggestion` is present. Email matching is exact + case-insensitive (WHMCS substring `search` results are filtered to a single exact email or no match).
